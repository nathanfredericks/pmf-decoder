...
define(
    "ENDIANNESS",
    pack("d", 1) === "\77\360\0\0\0\0\0\0" ? BIG_ENDIAN : LITTLE_ENDIAN
);
...
class Utils
{
    ...
    public static function getCallableIdentifier(callable $variable)
    {
        if (is_array($variable)) {
            return sha1(
                strtolower(get_class($variable[0])) .
                    "::" .
                    strtolower($variable[1])
            );
        } else {
    ...
    {
        $machine = php_uname("a");
        $machine .= file_exists("/proc/cpuinfo")
    ...
    public static function getIP($force = false)
    {
        ...
        if (
            ...
            if (
                preg_match('#">([0-9a-fA-F\:\.]*)</span>#', $ip, $matches) > 0
            ) {
                Utils::$ip = $matches[1];
            } else {
    ...
    public static function hexdump($bin)
    {
        ...
        foreach ($bin as $counter => $line) {
            $hex = chunk_split(
                chunk_split(
                    str_pad(bin2hex($line), 32, " ", STR_PAD_RIGHT),
                    2,
                    " "
                ),
                24,
            ...
            $output .=
                str_pad(dechex($counter << 4), 4, "0", STR_PAD_LEFT) .
                "  " .
                $hex .
                " " .
                $ascii .
    ...
    {
        return Utils::writeShort($item->getID()) .
            chr($item->count) .
            Utils::writeShort($item->getMetadata());
    }
    ...
    public static function readSlot($ob)
    {
        ...
        $cnt = ord($ob->get(1));
        return BlockAPI::getItem($id, Utils::readShort($ob->get(2)), $cnt);
    }
    ...
    public static function readDataArray($str, $len = 10, &$offset = null)
    {
        $data = [];
        $offset = 0;
        for ($i = 1; $i <= $len && isset($str[$offset]); ++$i) {
    ...
    public static function getRandomBytes(
        ...
        while (!isset($output[$length - 1])) {
            ...
            $weakEntropy = [
                is_array($startEntropy)
                    ? implode($startEntropy)
                    : $startEntropy,
            ...
            foreach ($weakEntropy as $k => $c) {
                ...
                $value ^=
                    hash("sha256", $c . microtime() . $k, true) .
                    hash("sha256", mt_rand() . microtime() . $k . $c, true);
                $value ^= hash(
                    "sha512",
                    ((string) lcg_value()) . $c . microtime() . $k,
                    true
                );
            }
            unset($weakEntropy);
                ...
                $strongEntropyValues = [
                    is_array($startEntropy)
                        ? hash(
                            "sha512",
                            $startEntropy[
                                ($rounds + $drop) % count($startEntropy)
                            ],
                            true
                        )
                        : hash("sha512", $startEntropy, true), //Get a random index of the startEntropy, or just read it
                    file_exists("/dev/urandom")
                        ? fread(fopen("/dev/urandom", "rb"), 64)
                        : str_repeat("\x00", 64),
                    (function_exists("openssl_random_pseudo_bytes") and
            ...
            $output .= substr(
                $value,
                0,
                min($length - strlen($output), $length)
            );
            unset($value);
            ++$rounds;
        }
    ...
    {
        $b = ord($c[0]);
        if ($signed === true and ($b & 0x80) === 0x80) {
